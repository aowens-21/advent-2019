...
; Used for part 2 to build initial state and "guess" noun and verb
(define (build-initial-state noun verb)
  (let ([state-vec (read-input-to-int-vector "input.txt")])
    (vector-set! state-vec 1 noun)
    (vector-set! state-vec 2 verb)
    state-vec))

; Brute force each possible combo until we reach the right one, BAD because it keeps doing work after it finds it
(define (find-correct-init-values)
  (let* ([possible-values (build-list 99 values)]
         [correct-values (list)])
    (for-each (lambda (possible-noun)
                (for-each (lambda (possible-verb)
                            (if (= (process-op (build-initial-state possible-noun possible-verb) 0 1 2 3) 19690720)
                                (set! correct-values (list possible-noun possible-verb))
                                #f))
                          possible-values))
              possible-values)
    correct-values))
